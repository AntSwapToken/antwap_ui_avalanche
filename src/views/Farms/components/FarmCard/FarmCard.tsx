...
import { Flex, Text, Skeleton } from 'avax-antswaptoken-uikit'
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethereum, account }) => {
  ...
  const totalValue: BigNumber = useMemo(() => {
    ...
    // Add all pairs
    if (farm.quoteTokenSymbol === QuoteToken.USDC_e) {
      return bnbPrice.times(farm.lpTotalInQuoteToken)
    }

    if (farm.quoteTokenSymbol === QuoteToken.USDT_e) {
      return bnbPrice.times(farm.lpTotalInQuoteToken)
    }

    if (farm.quoteTokenSymbol === QuoteToken.ANT) {
      return cakePrice.times(farm.lpTotalInQuoteToken)
    }
    ...
    if (farm.quoteTokenSymbol === QuoteToken.IRONICE) {
      return bnbPrice.times(farm.lpTotalInQuoteToken)
    }
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'ANT' && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
      ...
      <ExpandingWrapper expanded={showExpandableSection}>
        <DetailsSection
          ...
          bscScanAddress={
            farm.isTokenOnly
              ? `https://info.pangolin.exchange/token/${farm.tokenAddresses[process.env.REACT_APP_CHAIN_ID]}`
              : `https://info.pangolin.exchange/pair/${farm.lpAddresses[process.env.REACT_APP_CHAIN_ID]}`
          }
